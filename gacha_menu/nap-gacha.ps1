...
function Get-Gacha_os {
    Invoke-ScriptFromUrl -ScriptPath "gacha_clipboard/get_signal_link_os.ps1"
...
function Get-Gacha_cn {
    Invoke-ScriptFromUrl -ScriptPath "gacha_clipboard/get_signal_link_cn.ps1"
    Write-Host $Locale.TaskCompleted
    $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
    Close-Clear
}
...
function Get-Gacha_Cloud {
    Invoke-ScriptFromUrl -ScriptPath "gacha_clipboard/gacha_cloud_nap.ps1"
    Write-Host $Locale.TaskCompleted
    $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
    Close-Clear
}
