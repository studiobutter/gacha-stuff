# Script made by Star Rail Station under the Apache License. Remade by rng.moe for Zenless Zone Zero (China).
#
...
if ($set_path) {
    ...
    if (-Not [IO.File]::Exists($log_path)) {
        ...
        Write-Output "Try using the Global region script?"
        return
    }
